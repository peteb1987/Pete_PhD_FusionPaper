...
%
\begin{IEEEeqnarray}{rCl}
 \dot{x}(t) & = & \mathrm{f}(x(t), \tau_{K(t)}, u_{K(t)}) \label{eq:state_differential_eq}     .
\end{IEEEeqnarray}
...
\Delta \psi(t) &=& \frac{a_{N,K(t)}}{a_{T,K(t)}} \log \left( \frac{\dot{s}(t)}{\dot{s}_{K(t)}} \right)     .
\end{IEEEeqnarray}
...
\mathbf{r}(t) & = & \int_0^t \mathbf{v}(\tau) d\tau \nonumber \\
              & = & \mathbf{r}_{K(t)} + \frac{1}{a_{N,K(t)}^2 + 4 a_{T,K(t)}^2} \\
              & \times & \begin{bmatrix} \mathbf{e}_{T,K(t)} & \mathbf{e}_{N,K(t)} \end{bmatrix} \begin{bmatrix} \zeta_{1}(t) & \zeta_{2}(t) \\ \zeta_{2}(t) & -\zeta_{1}(t) \end{bmatrix}  \begin{bmatrix} 2 a_{T,K(t)} \\ a_{N,K(t)} \end{bmatrix} \nonumber \\
\zeta_{1}(t)  & = & \cos(\Delta \psi(t)) \dot{s}(t)^2 - \dot{s}_{K(t)}^2 \\
